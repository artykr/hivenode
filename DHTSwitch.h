...
class DHTSwitch : public SensorModule
{
  public:
    DHTSwitch(
      AppContext *context,            // We need a context structure to get global properties, e.g. method for push notifications on a switch change etc.
      ...
      double tThershold = 65535,       // Temperature thershold to switch the relay. 65535 value means no thershold.
      double hThershold = 65535,       // Humidity thershold to switch the relay.
      ...
      int8_t switchType = 1,          // 1 - turn on the relay if temperature/humdity is higher than thershold, 0 - vice versa.
      int8_t relayPin = -1
    );
  ...
  private:
    ...
    int8_t _switchType;           // 1 - switch on when crossing thershold from low to top
                                  // 0 - from top to low
    ...
    boolean _checkThershold();     // Check if t/h values are lower/higher than thershold level (depending on switchType)
