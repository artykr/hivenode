/*
  FallbackSwitch.h - Library for simple on/off driving a device with a relay
*/

#ifndef FallbackSwitch_h
#define FallbackSwitch_h
#define FALLBACKSWITCH_MODULE_VERSION 1

// TODO: change to const
#define FALLBACKSWITCH_RELAY_ON 0
#define FALLBACKSWITCH_RELAY_OFF 1
...
class FallbackSwitch : public SensorModule
{
  public:
    FallbackSwitch(AppContext *context, const byte zone, byte moduleId, int storagePointer, boolean loadSettings = true, int8_t switchPin = -1, int8_t devicePin = -1, int8_t fallbackPin = -1, boolean usePullup = false);
  ...
  private:
    ...
    int8_t _devicePin;            // Pin number for the light control (relay)
    int8_t _fallbackPin;
    ...
    boolean _usePullup;           // Use internal pullup resistors
